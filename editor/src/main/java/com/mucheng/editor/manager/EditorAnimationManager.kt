...
import com.mucheng.editor.animation.CursorVisibleAnimation
...
@Model
open class EditorAnimationManager(editor: MuCodeEditor) : AbstractManager(editor) {

    open var cursorAnimation: CursorAnimation = CursorTranslationAnimation(editor)
        protected set

    open var cursorVisibleAnimation: CursorVisible = CursorVisibleAnimation(editor)
        protected set

    @InvalidateRequired
    open fun setCursorAnimation(animation: CursorAnimation): EditorAnimationManager {
        this.cursorAnimation.cancel()
        this.cursorAnimation = animation
        return this
    }

    open fun setCursorVisibleAnimation(animation: CursorVisible): EditorAnimationManager {
        this.cursorVisibleAnimation.cancel()
        this.cursorVisibleAnimation = animation
        animation.start()
        return this
    }

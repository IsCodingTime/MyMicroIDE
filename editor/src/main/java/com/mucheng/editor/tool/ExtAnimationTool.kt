...
inline fun Cursor.executeAnimation(
    animation: CursorAnimation,
    start: Boolean = true,
    dispatchEvent: Boolean = true,
    block: () -> Unit
): CursorAnimation {
    animation.markAnimationBegin(line, row)
    block()
    animation.markAnimationFinish(line, row)
    if (dispatchEvent) {
        editor.eventManager.dispatchSelectionChangedEvent()
    }
    if (start) {
        animation.start()
    }
